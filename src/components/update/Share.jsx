...
const Share = ({ post, setShowPopupShare }) => {
    ...
    const handleSubmit = () => {
        if (post.is_deleted === 1) {
            toast.error('Bài viết đã bị xóa', {
                position: 'top-right',
            });
            return;
        }
        ...
        <div className="update">
            <div className='wrapper'>
                ...
                <div className='content-post border-1' >
                    {post.is_deleted === 1 ? <div>Bài viết đã bị xóa</div> :
                        <>
                            ...
                            <div className='main'>
                                <span style={{ display: 'flex', marginBottom: 8, fontWeight: 600 }}>{post.title}</span>
                                    ...
                                    <label htmlFor="image">
                                        <div className="imgContainer">
                                            {post.image &&
                                                <img
                                                    src={
                                                        post.category === 0 ? LINK_API_POST + (post.image ?? post.image)
                                                            : post.category === 1 ? LINK_API_AVATAR + (post.image ?? post.image)
                                                                : LINK_API_COVER + (post.image ?? post.image)
                                                    }
                                                    alt="ảnh"
                                                />
                                            }
